# Riley Ahlrichs        3-30-2025
# Assignment 4: Four in Sequence!
# This is an updated version of the classic board game Connect Four! This version is called Four in Sequence!
# The user can choose to play against a computer or against another person. The board will appear and each will take
# a turn to try to get four in a row (either diagonal, vertical or horizontal) and win!
...
def checkAdjacent(board, playerNumber):
    ...
    for column in range(len(board[0])):  # Iterate over all columns
        row = getOpenRow(board, column)  # Get the row where a piece can be placed

        if row != -1:  # If there is an open row in this column
            # Check all 8 adjacent directions: top-left, left, bottom-left, bottom, bottom-right, right, top-right
            directions = [
                (-1, -1),  # upper left
                (0, -1),   # left
                (1, -1),   # lower left
            ...
            for dr, dc in directions:
                ...
                if 0 <= new_row < len(board) and 0 <= new_col < len(board[0]):
                    # Check if the adjacent cell contains the same piece as the current player
                    if board[new_row][new_col] == piece:
                        adjacents.append(column)  # Append the column if adjacent piece is found

    # If adjacent columns are found, randomly select one
    if adjacents:
        # Print debugging information to understand the values being returned
        print(f"Adjacent columns found: {adjacents}")
...
def main():
    ...
    while running:
        ...
        if choice == "p":
            ...
            print("-The user selects a game between 1 or 2 players.")
            print("   -In 1-player mode, you play against the computer.")
            print("   -In 2-player mode, you take turns with another player.")
